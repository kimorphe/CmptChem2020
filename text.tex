...
\end{center}
\abstract{Abstract:
This study proposes a 2D coarse-grained molecular dynamics method for the numerical compaction 
simulation of montmorilonite clay moleculues. In this method, a unit structure of clay molecule 
with hydrated water is coarse-grained into a particle (CG-particle) to model the deformable 
molecule with tractable computational effort. Two kinds of inter-particle forces are considered 
in the simulations of CG particle system. The first is the intra-molecular force bonding the 
particle consisting a single moleculer. The 2nd is the inter-molecular van-der Waals force 
acting a pair of molecules. The former is given by a linear spring connetcting CG particles, 
while the latter by a Lenard-Jones(LJ) potential. 
The spring constants for the intra-molecular force are determined so as to 
proviede a reasonable rigidity to the molecule. The characterstics distance of the LJ 
potential is given depending on the amout of water  hydrated to the surfce of the molecule.
 With this model, compaction of water-hydrated clay is simulated. 
 The texture of the numerically compacted clay model is then evaluated by synthesizing 
 X-ray diffraction pattern as well as the radial distribution function. 
As the reults, it is shown that the experimentally observed XRD-peak can be 
reproduced by the preesent model. It is also found that the 4-6 molecules are 
 layered as a result of the analysis by the radial distribution function .
}
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\section{背景、目的}
